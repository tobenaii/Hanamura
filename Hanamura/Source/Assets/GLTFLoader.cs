...
namespace Hanamura
{
    ...
    {
        public record MeshData(string Name, LocalTransform Transform, VertexPositionNormalTexture[] Vertices, uint[] Indices, int[] Children)
        {
        }
        ...
        public static MeshData[] Load(string gltfPath)
        {
            ...
            foreach (var node in gltf.Nodes)
            {
                ...
                meshes[meshIndex] = new MeshData(gltf.Meshes[meshIndex].Name, transform, vertexData, uIndices, node.Children);
            }
